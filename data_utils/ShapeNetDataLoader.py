...
class PartNormalDataset(Dataset):
    def __init__(self, root, npoints, split='train', class_choice=None, normal_channel=False):
        ...
        self.root = root
        self.catfile = os.path.join(self.root, 'synsetoffset2category.txt') # category names -> folder names
        self.cat = {}
        ...
        self.cat = {k: v for k, v in self.cat.items()} # maybe because self.cat is not guaranteed to be a dict??
        self.classes_original = dict(zip(self.cat, range(len(self.cat)))) # {'Airplane': 0, 'Bag': 1, ...}
        ...
        for item in self.cat: # item = 'Airplane', 'Bag', ...
            # print('category', item)
            self.meta[item] = []
            dir_point = os.path.join(self.root, self.cat[item]) # <root>/02691156
        ...
        self.datapath = [] # [('Airplane', <root>/02691156/xxxxxx.txt), ('Bag', <root>/02773838/xxxxxx.txt), ...]
        for item in self.cat: # item = 'Airplane', 'Bag', ...
    ...
    def __getitem__(self, index):
        ...
        point_set[:, 0:3] = pc_normalize(point_set[:, 0:3]) # normalize to point cloud centroid
